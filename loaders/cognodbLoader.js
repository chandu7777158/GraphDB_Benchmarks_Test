...
export async function loadCognoDB(dataset) {
  ...
  console.log("\n🚀 [CognoDB] Starting data loading process...");
  const startTime = Date.now();

  try {
    // 1. Clean existing graph
    console.log("🧹 [CognoDB] Clearing old benchmark dataset...");
    await session.executeWrite((tx) =>
      tx.run("MATCH (n:User) DETACH DELETE n")
    );

    // 2. Create Indexes
    console.log("⚡ [CognoDB] Creating indexes on id and city...");
    try {
      await session.executeWrite((tx) =>
      ...
      console.warn("⚠️ [CognoDB] Index creation warning:", idxErr.message);
    }

    // 3. Load Nodes in Batches
    const nodeBatches = chunkArray(dataset.nodes, BATCH_SIZE);
    console.log(`📦 [CognoDB] Ingesting ${dataset.nodes.length.toLocaleString()} nodes in ${nodeBatches.length} batches...`);
    ...
    console.log(`🔗 [CognoDB] Ingesting ${dataset.relationships.length.toLocaleString()} relationships in ${relBatches.length} batches...`);
    ...
    console.log(`✅ [CognoDB] Load complete in ${(durationMs / 1000).toFixed(2)}s (${nodesPerSec} nodes/sec, ${relsPerSec} rels/sec)`);
    ...
    console.error("❌ [CognoDB] Data loading failed:", err.message);
    throw err;
  } finally {
    await session.close();
    await driver.close();
